...
def main(args):
    ...
    outfile = open("%s/RUST_tripeptide_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values), "w")
    ...
    try:
        ...
        infileopen = open("%s/RUST_tripeptide_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values))
        ax1_metafootprint= fig.add_subplot(111)   
        RUST_metagene_plot(infileopen, ax1_metafootprint)
        plt.savefig("%s/RUST_tripeptide_metafootprint_%s_%s_%s.png"%(args.Path, alignment_filename, args.offset, length_values))

    except:
        sys.stdout.write("Error producing images\n")
        
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Produces RUST metagene profile of tripeptide motifs')
    ...
    parser.add_argument('-P', '--Path', help='path to outputfile, default is "tripeptide"',default = "tripeptide")
