...
def main(args): 
    ...
    for transcript in list_transcripts :
        ...
        if average_gene_density != 0 :
            ...
            for sliding_w_n in range(0,len( elongation_region_part),3) :    #sliding window using increments of 3 nts
                ...
                codon_enrichment_expected_dict[amino_acid_1].append(expected_codon_density)
                    
                peptide_start += 1

    alignment_filename = args.alignment.split("/")[-1]
    if not os.path.exists(args.Path) : os.mkdir(args.o )
    outfile = open("%s/RUST_amino_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values), "w")
    ...
    for amino2 in list_amino_acids :
        ...
        for number in range(0,60) :
            if amino_enrichment_dict[amino2][number][0]  != 0 :
                outfile.write(  ", %s"%(amino_enrichment_dict[amino2][number][1]/amino_enrichment_dict[amino2][number][0]))
                list_data.append(amino_enrichment_dict[amino2][number][1]/amino_enrichment_dict[amino2][number][0])
            else :
                outfile.write(  ", 0")
                list_data.append(0)
        outfile.write(  "\n")
    ...
    try:
        ...
        infileopen = open("%s/RUST_amino_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values))
        ax1_metafootprint= fig.add_subplot(111)   
        RUST_metagene_plot(infileopen, ax1_metafootprint)
        plt.savefig("%s/RUST_amino_metafootprint_%s_%s_%s.png"%(args.Path, alignment_filename, args.offset, length_values))
    except:
        sys.stdout.write("Error producing images\n") 
        
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Produces RUST metagene profile of amino acids')
    ...
    parser.add_argument('-P', '--Path', help='path to outputfile, default is "amino"',default = "amino")
