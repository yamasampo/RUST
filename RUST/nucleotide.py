...
def main(args):
    ...
    outfile = open("%s/RUST_nucleotide_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values), "w")
    ...
    for nucleotide2 in nucleotides :
        ...
        outfile.write(", %s"%mean_value(nucleotide_enrichment_expected_dict[nucleotide2]))
        list_data = []
        for number in range(0,180) :
            if nucleotide_enrichment_dict[nucleotide2][number][0]  != 0 :
                ...
                outfile.write(", 0")
                list_data.append(0)
        rust_expected.append(mean_value(nucleotide_enrichment_expected_dict[nucleotide2]))
    ...
    try:
        ...
        infileopen = open("%s/RUST_nucleotide_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values))
        ax1_metafootprint= fig.add_subplot(111)   
        RUST_metagene_plot(infileopen, ax1_metafootprint)
        plt.savefig("%s/RUST_nucleotide_metafootprint_%s_%s_%s.png"%(args.Path, alignment_filename, args.offset, length_values))
    except:
        sys.stdout.write("Error producing images\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Produces RUST metagene profile of nucleotides')
    ...
    parser.add_argument('-P', '--Path', help='path to outputfile, default is "nucleotide"',default = "nucleotide")
