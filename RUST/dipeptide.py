...
def main(args):
    ...
    outfile = open("%s/RUST_dipeptide_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset,length_values), "w")
    ...
    for amino2 in list_amino_acids :
        ...
        if dipeptide_enrichment_expected_dict[amino2] != []:
            outfile.write( ", %s"%mean_value(dipeptide_enrichment_expected_dict[amino2]))
        list_data = []
    ...
    try :
        ...
        infileopen = open("%s/RUST_dipeptide_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset,length_values))
        ax1_metafootprint= fig.add_subplot(111)   
        RUST_metagene_plot(infileopen, ax1_metafootprint)
        plt.savefig("%s/RUST_dipeptide_metafootprint_%s_%s_%s.png"%(args.Path,alignment_filename, args.offset,length_values))
    
    except:
        sys.stdout.write("Error producing images\n")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Produces RUST metagene profile of dipeptides')
    ...
    parser.add_argument('-P', '--Path', help='path to outputfile, default is "dipeptide"',default = "dipeptide")
