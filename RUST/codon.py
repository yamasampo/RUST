...
def A_site_plot(infileopen35,dict_codon75,axis_Asite53,loc2):
    ...
    for line in infileopen35 :
        ...
        if amino in dict_amino_value:
            dict_amino_value[codon_to_amino_dict[linesplit[0]]].append(float(coverage_n[loc2]))
        else:
            dict_amino_value[codon_to_amino_dict[linesplit[0]]] = [float(coverage_n[loc2])]
    ...
    for _, amino_acid in list_amino_sorted:
        ...
        for amino_acid_codon in amino_to_codons_dict[amino_acid]:
            axis_Asite53.scatter(n1,A_site_value_norm_dict[amino_acid_codon],color = "gray", s=50, edgecolor = "gray")
...
def main(args):
    ...
    nts = ["A","G","C","T"]
    aligments_A1    = pysam.Samfile(args.alignment, 'rb')  #path to aligments in bam format
    ...
    list_10_percentile = []
    for value in range(1,10):list_10_percentile.append((len(list_transcripts)*value)/10)
    ...
    outfile = open("%s/RUST_codon_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values), "w")
    ...
    try:
        ...
        infileopen = open("%s/RUST_codon_file_%s_%s_%s"%(args.Path, alignment_filename, args.offset, length_values))
        ax1_metafootprint= fig.add_subplot(111)   
        RUST_metagene_plot(infileopen, ax1_metafootprint)
        plt.savefig("%s/RUST_codon_metafootprint_%s_%s_%s.png"%(args.Path, alignment_filename, args.offset, length_values))
        plt.clf()
        ...
        plt.savefig("%s/A_site_%s_%s_%s.png"%(args.Path, alignment_filename, args.offset, length_values))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Produces RUST metagene profile of codons')
    ...
    parser.add_argument('-P', '--Path', help='path to outputfile, default is "codon"',default = "codon")
